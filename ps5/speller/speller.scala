...
object Speller:
  ...
  def main(args: Array[String]): Unit = boundary:
    ...
    Zone:
      ...
      val timeLoad = calcTimeDiff(before, after)
      ...
      if file == null then // exit if failed
      ...
      while fread(c, sizeof[CChar], 1.toCSize, file) != 0 do
        ...
        if isalpha(!c) != 0 || (!c == '\'' && index > 0) then
          ...
          val misspelled = !Dictionary.check(word)
          getrusage(RUSAGE_SELF, after)
          timeCheck += calcTimeDiff(before, after) // Update benchmark
      ...
      getrusage(RUSAGE_SELF, after)
      val timeSize = calcTimeDiff(before, after)

      val total = timeLoad + timeCheck + timeSize
      printf(c"\n")
